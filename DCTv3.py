...
class DecisionTree:
    ...
    def construction(self, df, niveau = 0, method = "GINI", historique_attrib = []):
        ...
        if len(y.unique()) == 1 or niveau == self.taille_max or nb_ech < self.ech_min:
            if len(y.unique()) == 0:
                return Node(prediction=0) # S'il n'y a pas d'échantillons, retourne un nœud sans prédiction, ca ne marche pas vraiment comme je voudrais
